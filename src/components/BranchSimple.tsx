...
type SimpleStep = "channel" | "site" | "result";
...
export default function BranchSimple({
  ...
  function confirmCustomSite() {
    ...
    setSite({ id: -1, url: customUrl.trim(), tag: extractUrlSlug(customUrl.trim()) });
    setStep("result");
  }
  ...
  return (
    <section>
      ...
      {step === "result" && channel && site && (
        <StepResult channel={channel} site={site} />
      )}
...
function StepResult({ channel, site }: { channel: Channel; site: Site }) {
  // Кампания: если у канала есть default_content/term, считаем что они известны;
  // campaign возьмём из тега сайта (= URL slug для Zerocoder)
    ...
    <div>
      ...
      <EditableUtm
        initial={{
          ...
          campaign,
          content: channel.default_content ?? "",
